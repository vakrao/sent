...
def plot_community_network(
    ...
    """
    Draw the strongly-connected movement network, coloured by region and clustered by community.
    ...
    region_palette = _region_colormap(list(node_regions.values()))
    default_colour = "#bcbcbc"
    ...
    for comm_idx, nodes in enumerate(communities):
        ...
        colours = [
            region_palette.get(node_regions.get(n), default_colour) for n in nodes
        ]
    ...
    for region_id, colour in sorted(region_palette.items(), key=lambda x: x[0]):
    ...
    if node_regions and any(region is None for region in node_regions.values()):
        handles.append(
            plt.Line2D(
                [0],
                [0],
                marker="o",
                color="w",
                markerfacecolor=default_colour,
                markeredgecolor="#1f1f1f",
                markersize=7,
            )
        )
        labels.append("Region NA")
    ...
    ax.set_title(
        "Louvain Community Structure Coloured by Region",
        fontsize=14,
        pad=12,
    )
